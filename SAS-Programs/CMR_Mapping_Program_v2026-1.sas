...
/*                                                                */
/* Note:	    The SAS program CMR_Format_Program_v2026-1 must   */
...
DATA OUT1.&OUT.;
   ...
   DO I = 2 TO MIN(&MAXNDX,&NUMDX); 
      IF DX(I) NE " " THEN DO;
         ...
         /****************************************************/
         EXEMPTPOA(I) = 0;
         IF (ICDVER = 43 AND PUT(DX(I),$poaxmpt_v43fmt.)='1') OR
            (ICDVER = 42 AND PUT(DX(I),$poaxmpt_v42fmt.)='1') OR
            (ICDVER = 41 AND PUT(DX(I),$poaxmpt_v41fmt.)='1') OR
